...
function Unani() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:3001/api/product?category=Unani"
      );
    ...
    try {
      ...
      try {
        const diseasesResponse = await fetch(
          "http://localhost:3001/api/disease"
        );
  ...
  const applyFilters = () => {
    ...
    if (selectedDisease) {
      ...
      filtered = filtered.filter((product) => {
        ...
        if (product.description?.toLowerCase().includes(diseaseQuery))
          return true;
        if (product.tags && Array.isArray(product.tags)) {
          return product.tags.some(
            (tag) =>
              typeof tag === "string" &&
              tag.toLowerCase().includes(diseaseQuery)
          );
        }
        ...
        if (product.indications?.toLowerCase().includes(diseaseQuery))
          return true;
        return false;
      });
    }
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-center items-center h-64">
          <div className="text-lg text-gray-600">Loading Unani products...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4">
      ...
      <div className="mb-6">
        <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-600 mb-2">
          Unani Products
        </h1>
            ...
            <div className="flex items-center justify-center py-16 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200">
              <div className="text-center">
                ...
                <h3 className="text-xl font-medium text-gray-600 mb-2">
                  No Unani Products Found
                </h3>
                <p className="text-gray-500 text-sm max-w-md mb-4">
                  We couldn't find any Unani products matching your criteria.
                  Try adjusting your filters or search terms to discover
                  traditional Unani medicines.
                </p>
                <button
