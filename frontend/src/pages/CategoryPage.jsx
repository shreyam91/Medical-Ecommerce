...
function CategoryPage() {
  ...
  useEffect(() => {
    setLoading(true)
    setError(null)
    fetch(`http://localhost:3001/api/product?category=${encodeURIComponent(categorySlug)}`)
      .then(res => {
  ...
  // Filtering by brand/disease (assuming product.brand_id and product.disease_id)
    ...
    <div className="p-4 max-w-7xl mx-auto">
      <div className="flex gap-6">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold mb-6 capitalize">{categorySlug.replace(/-/g, ' ')}</h1>
